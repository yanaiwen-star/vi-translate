...
| 模式 | 说明 | 收费 |
|------|------|------|
| 🎙️ **同传** | 实时同声传译：边说边出译文，基于 WebSocket 流式链路 | 注册即送 30 分钟（一次性赠送，用完不重置），超出可购语音包 |
...
- 只有实时同传用完赠送时长后计费（新人注册即送 30 分钟，一次性、终身累计、用完不重置；绑定昵称 +10 分钟、绑定手机 +20 分钟，封顶 60 分钟）。超出可购语音包 ¥9.9 / ¥19.9 / ¥49.9，对应 60 / 200 / 600 分钟，一次购买长期有效
...
| `FREE_TOTAL_MINUTES` | 注册赠送的同传分钟数，默认 30（一次性赠送、终身累计、用完不重置，非每日额度） |
